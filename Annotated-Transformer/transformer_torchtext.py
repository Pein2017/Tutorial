...
def example_positional():
    ...
    return (alt.Chart(data).mark_line().properties(width=800).encode(
        x="position", y="embedding", color="dimension:N").interactive())
...
def load_tokenizers():
    ...
    try:
        ...
        print(f'downloading de_core_news_sm')
        os.system("python -m spacy download de_core_news_sm")
        spacy_de = spacy.load("de_core_news_sm")
        print(f'de_core_news_sm loaded ')

    try:
        spacy_en = spacy.load("en_core_web_sm")
    except IOError:
        print(f'downloading en_core_web_sm')
...
def train_worker(npu,
                 ...
                 dist_url=None):
    print(f"Train worker process using NPU: {npu} for training", flush=True)
    ...
    if is_distributed:
        dist.init_process_group(backend='hccl',
                                ...
                                rank=npu,
                                world_size=nnpus_per_node)
        model = torch.nn.parallel.DistributedDataParallel(model,
                                                          device_ids=[npu])
        module = model.module
        is_main_process = npu == 0
    ...
    train_dataloader, valid_dataloader = create_dataloaders(
        ...
        spacy_de,
        spacy_en,
        batch_size=config["batch_size"] // nnpus_per_node,
    ...
    for epoch in range(config["num_epochs"]):
        ...
        model.train()
        print(f"[NPU{npu}] Epoch {epoch} Training ====", flush=True)
        ...
        print(f"[NPU:{npu}] Epoch {epoch} Validation ====", flush=True)
        model.eval()
        sloss = run_epoch(
...
def train_distributed_model(vocab_src, vocab_tgt, spacy_de, spacy_en, config):
    ...
    nnpus_per_node = torch.npu.device_count()
    print(f"Number of NPUs detected: {nnpus_per_node}")
    print("Spawning training processes ...")
    mp.spawn(
        train_worker,
        nprocs=nnpus_per_node,
        args=(nnpus_per_node, vocab_src, vocab_tgt, spacy_de, spacy_en, config,
              True, dist_url),
    )


def train_model(vocab_src, vocab_tgt, spacy_de, spacy_en, config, device_id=0):
    if config["distributed"]:
        train_distributed_model(vocab_src, vocab_tgt, spacy_de, spacy_en,
                                config)
    else:
        train_worker(device_id, 1, vocab_src, vocab_tgt, spacy_de, spacy_en,
                     config, False)
...
def load_trained_model():
    ...
    if not exists(model_path) or not config["load_model"]:
